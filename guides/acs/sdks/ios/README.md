...
1. Drag the **Appcelerator.framework** folder from the **appcelerator-sdk-ios-1.0.0/** folder into your project's Frameworks folder, making sure Copy items into destination is checked, and click Finish.
...
4. On your project's **Build Settings** tab, click **All** button in the top-left corner of the tab. Then expand the Linking section and add **-ObjC** to the **Other Linker Flags** field.

5. Import Appcelerator.h into your application delegate:

        #import <Appcelerator/Appcelerator.h>

6. Lastly, initialize Platform services, replacing **<<YOUR APP KEY >>** with application key generated by
...
The second parameter of each method call is a block that is passed an [APSResponse](http://docs.appcelerator.com/aps-sdk-apidoc/latest/ios/Classes/APSResponse.html) that has properties that contain information about the response. For instance,
...
is a code block that is periodically invoked and passed a float value indicating the progress of the request, and a boolean indicating if the request is for an upload (YES) or download (NO).

#### Example: APSPhotos Create Call with Progress Handler

The following example creates a new Photo object from a binary photo attachment.

The `progressBlock` code block sets the `progress ` property on a `ProgressBar` object, displaying the 
...
The [\[APSCloud sendRequest\]](http://docs.appcelerator.com/aps-sdk-apidoc/latest/ios/Classes/APSCloud.html#//api/name/sendRequest:method:data:handler:) method lets you easily make REST API calls directly against
ACS, rather than using the specialized classes. In general, you
...
The following example calls [users/login.json](http://docs.appcelerator.com/cloud/latest/#!/api/Users-method-login) REST method directly and logs the result to the console.
