...
The sizes are generated asynchronously, so the API or SDK call will return
...
processed, the `processed` flag will be set to `true` and the URLs which can
be used to access the photos will be shown:
